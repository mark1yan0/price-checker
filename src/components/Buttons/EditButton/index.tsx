'use client';

import { useState } from 'react';
import Button from '..';
import { updateItemAction } from '@/app/actions';
import { IPriceItem } from '@/types/database';
import Form from '@/components/Form';

const EditButton = ({ item }: { item: IPriceItem }) => {
  const [opened, setOpened] = useState(false);
  return (
    <>
      <Button onClick={() => setOpened(!opened)} variant="neutral">
        Edit {opened ? item.id : null}
      </Button>

      <Sidebar opened={opened} onClose={() => setOpened(false)}>
        <UpdateItemForm item={item} />
      </Sidebar>
    </>
  );
};

export default EditButton;

const UpdateItemForm = ({ item }: { item: IPriceItem }) => {
  const updateItemActionWithId = updateItemAction.bind(null, item.id);
  return (
    <Form
      initialValues={{
        id: item.id,
        name: item.name,
        url: item.url,
        css_selector: item.css_selector,
        base_price: item.base_price,
      }}
      columns={1}
      action={updateItemActionWithId}
    />
  );
};

const Sidebar = ({
  opened,
  onClose,
  children,
}: {
  opened: boolean;
  onClose: (event: React.MouseEvent<HTMLButtonElement>) => void;
  children: React.ReactNode;
}) => {
  if (!opened) {
    return null;
  }

  return (
    <div className="-bg--color-surface-200 absolute right-0 top-0 h-full min-w-full p-4 md:min-w-[500px]">
      <div className="flex w-full justify-end">
        <button onClick={onClose} className="group rounded p-1 hover:bg-white">
          <svg
            width="15"
            height="15"
            viewBox="0 0 15 15"
            fill="none"
            className="group-hover:-stroke--color-surface-200"
            xmlns="http://www.w3.org/2000/svg"
          >
            <path
              d="M12.8536 2.85355C13.0488 2.65829 13.0488 2.34171 12.8536 2.14645C12.6583 1.95118 12.3417 1.95118 12.1464 2.14645L7.5 6.79289L2.85355 2.14645C2.65829 1.95118 2.34171 1.95118 2.14645 2.14645C1.95118 2.34171 1.95118 2.65829 2.14645 2.85355L6.79289 7.5L2.14645 12.1464C1.95118 12.3417 1.95118 12.6583 2.14645 12.8536C2.34171 13.0488 2.65829 13.0488 2.85355 12.8536L7.5 8.20711L12.1464 12.8536C12.3417 13.0488 12.6583 13.0488 12.8536 12.8536C13.0488 12.6583 13.0488 12.3417 12.8536 12.1464L8.20711 7.5L12.8536 2.85355Z"
              fill="currentColor"
              fillRule="evenodd"
              clipRule="evenodd"
            ></path>
          </svg>
        </button>
      </div>
      {children}
    </div>
  );
};
